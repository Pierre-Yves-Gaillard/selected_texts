...
$$
$$
=\frac{1}{2\pi i}\int_\gamma\ \frac{f(z)}{z-b}\ \sum_{\lambda\in\Lambda(a)}1_\lambda\ dz
$$
$$
=\sum_{\lambda\in\Lambda(a)}\frac{1_\lambda}{2\pi i}\int_\gamma\ \frac{f(z)}{z-b}\ dz
$$
$$
=\sum_{\lambda\in\Lambda(a)}\frac{1_\lambda}{2\pi i}\int_\gamma\ \frac{f(z)\ dz}{(z-\lambda)-(b-\lambda)}
$$
$$
=\sum_{\lambda\in\Lambda(a)}\frac{1_\lambda}{2\pi i}\int_\gamma\ f(z)\sum_{k=0}^{n-1}
\frac{(b-\lambda)^k}{(z-\lambda)^{k+1}}\ dz
$$
$$
=\sum_{\lambda\in\Lambda(a),0\le k<n}\frac{1_\lambda}{2\pi i}\int_\gamma\ \frac{f(z)\ dz}{(z-\lambda)^{k+1}}\ (b-\lambda)^k
$$
$$
\overset{(*)}{=}\sum_{\lambda\in\Lambda(a),0\le k<n}I(\gamma,\lambda)\ \frac{f^{(k)}(\lambda)_\lambda}{k!}\ (b-\lambda)^k
$$
$$
=\sum_{\lambda\in\Lambda(a),0\le k<n}\frac{f^{(k)}(\lambda)_\lambda}{k!}\ (b-\lambda)^k
$$
$$
\overset{(**)}{=}f(b),
$$
...
\href{https://www.writelatex.com/747360qwgtbf}{https://www.writelatex.com/747360qwgtbf} 
