...
\newpage 
% 
\section{The Fundamental Theorem of Galois Theory (FTGT)} 
%
\n{\footnotesize\textbf{Abstract.} We give a short and self-contained proof of the Fundamental Theorem of Galois Theory (FTGT) for finite degree extensions.}\so

\n We derive the FTGT (for finite degree extensions) from two statements, denoted (a) and (b). These two statements, and the way they are proved here, go back at least to Emil Artin (precise references are given below).
...
\n Let $E/F$ be an extension of fields, let $a_1,\dots,a_n$ be distinct generators of $E/F$ such that $p:=(X-a_1)\cdots(X-a_n)$ is in $F[X]$. Then
...
\n\textbf{Proof that (a) and (b) imply the theorem.} Let $S/F$ be a sub-extension of $E/F$ and put $H:=\text{Aut}(E/S)$. Then we have trivially $S\subset E^H$, and (a) and (b) imply 
$$
[E:S]=[E:E^H].
$$
...
\n\textbf{Proof of (a).} Let $1\le i\le n$. Put $K:=S[a_1,\dots,a_{i-1}]$ and $d:=[K[a_i]:K]$. It suffices to check that any $F$-embedding $\phi$ of $K$ in $E$ has exactly $d$ extensions to an $F$-embedding $\Phi$ of $K[a_i]$ in $E$. Let $q\in K[X]$ be the minimal polynomial of $a_i$ over $K$. It is enough to verify that $\phi(q)$ (the image under $\phi$ of $q$) has $d$ distinct roots in $E$. But this is clear since $q$ divides $p$, and thus $\phi(q)$ divides $\phi(p)=p$.

\n\textbf{Proof of (b).} In view of (a) it is enough to check $|H|\ge[E:E^H]$. Let $k$ be an integer larger than $|H|$, and pick a 
$$
...
\n[A] Emil Artin, Galois Theory, Lectures Delivered at the University of Notre Dame, Chapter II: 
...
For any matrix $a$ in $A:=M_n(\C)$, write $\Lambda(a)$ for the set of eigenvalues of $a$. Let $U$ be an open subset of $\C$, and let $U'$ be the subset of $A$, which is open by Rouch\'e's Theorem, defined by the condition $\Lambda(a)\subset U$. Let $a$ be in $U'$, let $X$ be an indeterminate, and let $\mathcal O(U)$ be the $\C$-algebra of holomorphic functions on $U$. Equip $\mathcal O(U)$ and $\C[a]$ with the $\C[X]$-algebra structures associated respectively with the element $z\mapsto z$ of $\mathcal O(U)$ and the element $a$ of $\C[a]$. 

\n\textbf{Theorem.} (i) \emph{There is a unique $\C[X]$-algebra morphism from $\mathcal O(U)$ to $\C[a]$. We denote this morphism by $f\mapsto f(a)$.}

\n(ii) \emph{There is an $r>0$ and a neighborhood $N$ of $a$ in $A$ such that 
$$
f(b)=\frac{1}{2\pi i}\ \sum_{\lambda\in\Lambda(a)}\ \int_{|z-\lambda|=r}\ \frac{f(z)}{z-b}\ dz
$$
for all $f$ in $\mathcal O(U)$ and all $b$ in $N$. In particular the map $b\mapsto f(b)$ from $U'$ to $A$ is holomorphic.}

\n\textbf{Proof.} By the Chinese Remainder Theorem, $\C[a]$ is isomorphic to the product of $\C[X]$-algebras of the form $\C[X]/(X-\lambda)^m$, with $\lambda\in\C$. So we can assume that $\C[a]$ is of this form, and (i) is clear. To prove (ii) we can keep on assuming $\C[a]\simeq\C[X]/(X-\lambda)^m$. On replacing $a$ with $a-\lambda$, we can even assume $a^n=0$. Choose $r>0$ so that $U$ contains the closed disk of radius $r$ centered at $0$, let $N$ be the set of those $b$ in $A$ whose eigenvalues $\lambda$ satisfy $|\lambda|<r/2$, and let $b$ be in $N$. Replacing $a$ with $b$ in the above argument, we can assume $b^n=0$. Now (ii) follows from Cauchy's Integral Formula and the equalities
$$
f(b)=\sum_{k=0}^{n-1}\ \frac{f^{(k)}(0)}{k!}\ b^k,\quad 
\frac{1}{z-b}=\sum_{k=0}^{n-1}\ \frac{b^k}{z^{k+1}}\quad.
$$ 
% 
\newpage 
%
...
%
\n{\footnotesize\textbf{Abstract.} We give a short proof of Zorn's Lemma.}
...
\n[\S\ 41, p. 195, Kleene S.; \textbf{Introduction to Metamathematics}, New York, Van Nostrand 1952.] 
% 
\newpage 
% 
\n This text is available in tex and/or pdf format 

\n$\bullet$ on my web page: \href{http://iecl.univ-lorraine.fr/~Pierre-Yves.Gaillard/}{http://iecl.univ-lorraine.fr/$\sim$Pierre-Yves.Gaillard/}, 

\n$\bullet$ at figshare in pdf format:\\\href{http://figshare.com/articles/Selected_Texts_pdf_/731674}{http://figshare.com/articles/Selected\_Texts\_pdf\_/731674} 

\n$\bullet$ at figshare in tex format:\\\href{http://figshare.com/articles/Selected_Texts_tex_/731673}{http://figshare.com/articles/Selected\_Texts\_tex\_/731673} 

\n$\bullet$ at github in tex format:\\\href{https://github.com/Pierre-Yves-Gaillard/selected_texts}{https://github.com/Pierre-Yves-Gaillard/selected\_texts} 

\n$\bullet$ at writeLaTeX in tex and pdf format:\\\href{https://www.writelatex.com/245642vdhjtd}{https://www.writelatex.com/245642vdhjtd} 

\n$\bullet$ at Google document in tex and df format: \href{http://goo.gl/sWG1la}{http://goo.gl/sWG1la} 

\n$\bullet$ at WordPress in pdf format:\\\href{http://gaillardpy.wordpress.com/2013/06/26/selected-texts/}{http://gaillardpy.wordpress.com/2013/06/26/selected-texts/} 
% 
\end{document} 
