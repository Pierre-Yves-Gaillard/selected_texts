...
For any pair $I\subset S$ of subsets of $P$, say that $I$ is an \textbf{initial segment} of $S$ if $S\ni s<i\in I$ implies $s\in I$. 

For any well-ordered subset $W$ of $P$ choose an element $p(W)$ in $P_{>W}$, \emph{i.e.} $p(W)\in P$ and $p(W)>w$ for all $w$ in $W$. Let $\mathcal W$ be the set of those well-ordered subsets $W$ of $P$ such that $p(W_{<w})=w$ [self-explanatory notation] for all $w$ in $W$, and let $U\subset P$ be the union of $\mathcal W$. 
...
Item (c) will give the contradiction $U\cup\{p(U)\}\in\mathcal W$.
...
We prove (c). To check that $U$ is well-ordered, let $A$ be a nonempty subset of $U$, choose a $W$ in $\mathcal W$ which meets $A$, let $m$ be the minimum of $W\cap A$, and let $a$ be in $A$. We must show $m\le a$. If such was not the case, (a) would imply $a<m$, in contradiction with (b). So, $U$ is well-ordered, and it only remains to prove $p(U_{<u})=u$ for $u$ in $U$, that is, $U_{<u}\subset W$ for $u\in W\in\mathcal W$. This follows from (b).
